...
export default class Parser {
  ...
  Literal() {
    ...
    throw new SyntaxError(
      `Literal: unexpected literal production "${
        ...
        null,
        2
      )}", next: ${JSON.stringify(this._tokenizer.exec()[0])}`
    );
  }
  ...
  NumericLiteral(type) {
    let token;
    if (type === "INTEGER") token = this._eat("INTEGER");
    else if (type === "FLOAT") token = this._eat("FLOAT");
    return {
      type: "NumericLiteral",
      valType: type,
  ...
  _eat(tokenType) {
    ...
    if (token == null) {
      throw new SyntaxError(
        `Unexpected end of input, expected: "${tokenType}", last: "${JSON.stringify(
          this._last,
          null,
          2
        )}", next: ${JSON.stringify(this._tokenizer.exec()[0])}`
    ...
    if (token.type !== tokenType) {
      throw new SyntaxError(
        `Unexpected token: "${
          token.value
        }", expected: "${tokenType}", last: "${JSON.stringify(
          this._last,
          null,
          2
        )}", next: ${JSON.stringify(this._tokenizer.exec()[0])}`
