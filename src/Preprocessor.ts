...
export default class Preprocessor {
  ...
  preprocess(mainFile, _symbols = null) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      const lookahead = [tokens[i + 1], tokens[i + 2]];
      ...
      if (tokens[i].type === "PRE_DEFINE") {
        if (lookahead[0].type === "IDENTIFIER") {
          ...
          throw new PreprocessorError(`Unexpected token "${JSON.stringify(
            lookahead[0]
          )}", 
        ...
        if (lookahead[0].type === "FILENAME") {
          const src = readFileSync(
            this.getDependencyPath(lookahead[0].value),
            "utf-8"
          );
        ...
        String(tokens[i].value) in symbols
      ) {
