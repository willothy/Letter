...
export default class Compiler {
  ...
  handleNumericTypecasts(value, expectedType, gotType) {
    if (expectedType === "Double" && gotType === "Float") {
      ...
      return this.builder.CreateFPTrunc(
        value,
        this.builder.getDoubleTy(),
        "dbl_downcast"
      );
    } else if (expectedType === "Double" && gotType === "Integer") {
      return this.builder.CreateSIToFP(
        value,
        this.builder.getDoubleTy(),
        "si_to_dbl"
      );
      ...
      (gotType === "Float" || gotType === "Double")
    ) {
      return this.builder.CreateFPToSI(
        value,
        this.builder.getInt64Ty(),
        "flt_to_si"
      );
  ...
  unbackslash(s) {
    return s.replace(/\\([\\rnt'"])/g, function (match, p1) {
      const codes = [];
      for (const letter of p1) codes.push(letter.charCodeAt(0));
  ...
  codegen(ast, symbols = {}, fn = null) {
    ...
    if (current.type === "FunctionDeclaration") {
      ...
      const returnType =
        current.returnType.type === "void"
          ? this.builder.getVoidTy()
          : this.resolveFuncType(current.returnType);
      ...
      if (current.returnType.type === "void") this.builder.CreateRetVoid();
    }
      ...
      for (const declaration of current.declarations) {
        ...
        if (declaration.init) {
          ...
          this.builder.CreateStore(
            this.checkType(this.codegen(declaration.init, symbols), type),
            alloc
          );
        }
      }
    ...
    if (current.type === "AssignmentExpression") {
      ...
      this.builder.CreateStore(
        this.checkType(this.codegen(current.right, symbols, fn), info.type),
        info.alloc
      );
      return this.builder.CreateLoad(info.type, info.alloc, current.left.name);
    }
    if (current.type === "NumericLiteral") {
      return current.valType === "INTEGER"
        ? ConstantInt.get(this.builder.getInt32Ty(), current.value, true)
        : ConstantFP.get(this.builder.getFloatTy(), current.value);
    }
    ...
    if (current.type === "StringLiteral") {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        const insideElementPtr = this.builder.CreateGEP(arrayType, alloc, [
        ...
        this.builder.CreateStore(
          ConstantInt.get(
            this.context,
            new APInt(8, char.charCodeAt(0), false)
          ),
          insideElementPtr
        );
      }
      return this.builder.CreateGEP(arrayType, alloc, [
        this.builder.getInt32(0),
        this.builder.getInt32(0),
      ]);
    }
    if (current.type === "Identifier") {
      ...
      return info.isArg
        ? info.alloc
        : this.builder.CreateLoad(info.type, info.alloc, "tmp");
    }
    if (current.type === "BinaryExpression") {
      ...
      if (Type.isSameType(leftType, rightType)) {
        ...
        if (this.isFloat(left) || this.isFloat(right)) {
          left = this.isFloat(left)
            ? left
            : this.builder.CreateSIToFP(
                left,
                this.builder.getFloatTy(),
                "convleft"
              );
          right = this.isFloat(right)
            ? right
            : this.builder.CreateSIToFP(
                right,
                this.builder.getFloatTy(),
