...
define double @circumferenced(double %radius) {
entry:
  %flt_multmp = fmul double 0x401921FB4D12D84A, %radius
  ret double %flt_multmp
}

define float @circumferencef(float %radius) {
entry:
...
entry:
  ...
  %0 = call float @circumferencef(float 1.000000e+01)
  ...
  %1 = call double @circumferenced(double 1.000000e+01)
  store double %1, double* %d, align 8
  %id_load_tmp1 = load double, double* %v, align 8
  call void @printDouble(double %id_load_tmp1)
  %id_load_tmp2 = load double, double* %d, align 8
  call void @printDouble(double %id_load_tmp2)
  ret i32 0
}
