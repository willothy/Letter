...
define float @circumference.1(float %radius) {
entry:
  %flt_multmp = fmul float 0x401921FB40000000, %radius
  ret float %flt_multmp
}

define void @printf.1(double %f) {
entry:
...
entry:
  ...
  %0 = call float @circumference.1(float 1.000000e+01)
  ...
  call void @printf.1(double %id_load_tmp1)
